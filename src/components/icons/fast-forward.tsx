import { BaseComponent } from 'lib/types/component'
import React from 'react'

interface FastForwardIconProps extends BaseComponent<HTMLOrSVGElement> {}

const FastForwardIcon: React.FC<FastForwardIconProps> = props => {
	const { children, ...attr } = props
	return (
		<svg
			xmlns="http://www.w3.org/2000/svg"
			viewBox="0 0 321.9 321.9"
			fill="currentColor"
			{...attr}
		>
			<path
				d="M315.6,149.301l-147.1-98.2c-2-1.1-4.6-2.2-7.2-2.2v0c-7.9,0-14.2,6.3-14.2,14.2v71.9l-125.5-84c-2.1-1.1-4.7-2.1-7.4-2.1
c-7.9,0-14.2,6.3-14.2,14.2v195.9c0,11,12.1,17.9,21.5,11.6l125.5-83.5v71.9c0,11,12.1,17.9,21.5,11.6l147.1-97.7
C324,167.101,324,155.001,315.6,149.301z M27.8,89.201l107.7,71.5l-107.7,71.9V89.201z M174.9,89.201l107.7,71.5l-107.7,71.9
V89.201z"
			/>
		</svg>
	)
}
export default FastForwardIcon
